...
| 날짜 | 내용 |
|------|------|
| 2026-04-19 | 📋 **명함 정보 밀도 강화** — 부서/팀, 영문 태그라인, 핵심 역량 칩 3개, 가상 수상, 가상 주소 추가 (Gemini JSON에 4개 필드 추가, 추가 비용 0) → 빈 공간 없애고 진짜 명함처럼 구성 |
| 2026-04-19 | 🎭 **직업 이모지 자동 매핑** — 사진 미업로드 시 아바타에 AI가 추천한 단일 이모지 표시 (디자이너→🎨, 우주비행사→🚀 등) · 추가 비용 0 (Gemini Text JSON 응답에 emoji 필드 추가) |
| 2026-04-19 | 🐛 **이름 fake bold 수정** — `font-weight: 900`이 캡처 시 시스템 fake bold로 흐릿해지던 이슈 → 800 + `document.fonts.load("800 36px Pretendard","한글")` 트리거 + onclone 강제 스타일 |
| 2026-04-19 | 📷 **프로필 사진 업로드** 추가 (4MB 이하, 선택) + 좌측 사이드바 130px 확장 + 90px 원형 아바타 + 흰색 inset 보더 |
| 2026-04-19 | 🎓 **「초등학생」** 신분 옵션 추가 (맨 앞에 배치 — 학교 부스 비율 반영) |
| 2026-04-19 | 🔧 **PNG 캡처 깨짐 수정** — 좌측 세로 회전 텍스트가 카드 밖으로 잘리던 문제 → 가로 배지 (`FUTURE 2036`)로 교체 |
